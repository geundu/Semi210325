	...
	protected void broadcast(String msg) {
		...
		synchronized (this) {
			...
			for (int i = 0; i < ts.roomList.size(); i++) {
				...
				if (g_title.equals(room.getTitle())) {
...
//					room.setCurrent(Integer.parseInt(currentNum));
				...
				case Protocol.ROOM_IN: {
					...
					for (int i = 0; i < ts.roomList.size(); i++) {
						...
						if (roomTitle.equals(room.getTitle())) {
							g_title = roomTitle;
							g_current = room.getCurrent() + 1;
							// 이코드 없으면 인원수 업데이트가 안됨.
							room.setCurrent(g_current);
							room.userList.add(this);
							JOptionPane.showMessageDialog(ts, "tst.globalList" + ts.globalList + " room.userList"
														+ room.userList);
							room.idList.add(nickName);
					...
					for (int i = 0; i < ts.roomList.size(); i++) {// 방갯수
						...
						for (int j = 0; j < room.idList.size(); j++) {
							...
							if (!nickName.equals(room.idList.get(j))) {
